...
for a Cosmos Cloud Mastodon instance using the `tootctl` command-line tool running in the Cosmos Mastodon container.

Example usage:
    python3 create_users_cosmos_mastodon.py testemail -n 3 --dry-run
    This will simulate creating 3 users without actually making any changes

    python3 create_users_cosmos_mastodon.py youremail -n 5
    This will create 5 users with email addresses like youremail+user0001@gmail.com

    python3 create_users_cosmos_mastodon.py youremail -n 5 --creds-file custom_creds.txt
...
def approve_user(username: str, dry_run: bool = False) -> tuple[bool, str]:
    ...
    try:
        output = subprocess.check_output(
            ["tootctl", "accounts", "approve", username], stderr=subprocess.STDOUT, text=True
        )
